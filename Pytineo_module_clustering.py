...
"""
                                                                   ## ne pas faire apparaitre les messages de type Warning
...
def StartPoint(nom_commune_reference, duree_du_sejour, dict_themes, dict_sous_themes, df_POI, dict_parametres_techniques):
   ...
   ## si la commune sélectionnée comporte au moins 10 POI, poursuite du traitement
    if df_POI_zoom_sur_centroid.shape[0] > 10:
        POI_disponibles_sur_commune = True

        ## Recensement de la répartition potentielle des itinéraires par implémentation de la méthode des KMEANS
        if duree_du_sejour > 2:
            df_POI_zoom_sur_centroid, dict_final_centroids_nbre_itineraires = affectation_itineraire_aux_centroids(
                df_POI_zoom_sur_centroid, duree_du_sejour, nom_commune_reference)
        else:
            ## uniquement autour de la commune choisie (si séjour < 3 jours)
            df_POI_zoom_sur_centroid['Numéro_centroïd'] = 0
            dict_final_centroids_nbre_itineraires = {}
            liste_temporaire = []
            liste_temporaire.append(lat_centre_commune_degre)
            liste_temporaire.append(lon_centre_commune_degre)
            liste_temporaire.append(duree_du_sejour)
            dict_final_centroids_nbre_itineraires[0] = liste_temporaire
       ...
       ## Eclatement du dataframe source par centroïd
        ...
        for cle, valeur in dict_final_centroids_nbre_itineraires.items():
           ##liste_df_POI_zoom_sur_centroid.append(df_POI_zoom_sur_centroid[df_POI_zoom_sur_centroid['Numéro_centroïd'] == cle])
            dict_df_POI_zoom_sur_centroid[cle] = df_POI_zoom_sur_centroid[df_POI_zoom_sur_centroid['Numéro_centroïd'] == cle]
            dict_attributs_sejour = {'nom_commune_reference': nom_commune_reference,
                                     'lat_centre_commune_degre': lat_centre_commune_degre,
                                     'lon_centre_commune_degre': lon_centre_commune_degre,
                                     'Restauration': 'Restauration', 'Restauration souhaitee': valeur_restau_bar_theme,
                                     'Restauration rapide': 'Restauration rapide',
                                     'Restauration rapide souhaitee': valeur_restau_rapide,
                                     'Gastronomie': 'Gastronomie', 'Gastronomie souhaitee': valeur_gastronomie,
                                     'Nombre max POI resto-gastro souhaite': nbre_POI_resto_dans_perimetre_iti}

    else:
       ## si pas suffisamment de POI, pas de construction d'itinéraires
        POI_disponibles_sur_commune = False
        dict_final_centroids_nbre_itineraires, dict_df_POI_zoom_sur_centroid, dict_attributs_sejour = {}, {}, {}

    return POI_disponibles_sur_commune, dict_final_centroids_nbre_itineraires, dict_df_POI_zoom_sur_centroid, dict_attributs_sejour
